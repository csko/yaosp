...
int schedule( registers_t* regs ) {
    ...
    /* Set the base of the descriptor to the address of the TLD of the new thread. */
    gs = (GDT_ENTRIES + MAX_CPU_COUNT + get_processor_index()) * 8;
    gdt_set_descriptor_base(gs, (uint32_t)next->tld_data);
    ...
    switch_to_thread(arch_thread->esp, gs);
