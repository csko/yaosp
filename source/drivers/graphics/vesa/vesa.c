...
static int detect_vesa( void ) {
    ...
    memory_region_t* info_region;

    uint16_t* tmp;
    ptr_t mode_addr;
    uint16_t* mode_ptr;

    info_region = memory_region_create( "VESA info", PAGE_SIZE, REGION_KERNEL | REGION_READ | REGION_WRITE );

    if ( info_region == NULL ) {
        return -ENOSPC;
    }

    memory_region_remap_pages( info_region, 0x2000 );
    vesa_info = ( vesa_info_t* )info_region->address;
    ...
    mode_addr = ( ptr_t )( ( ( vesa_info->video_mode_ptr & 0xFFFF0000 ) >> 12 ) |
                           ( vesa_info->video_mode_ptr & 0xFFFF ) );

    memory_region_put( info_region );

    info_region = memory_region_create( "VESA mode table", PAGE_SIZE, REGION_KERNEL | REGION_READ );

    if ( info_region == NULL ) {
        return -1;
    }

    memory_region_remap_pages( info_region, mode_addr & PAGE_MASK );
    mode_ptr = ( uint16_t* )( info_region->address + ( mode_addr & ~PAGE_MASK ) );
...
static int vesa_get_mode_information( uint16_t mode_number, vesa_mode_info_t* mode_info ) {
    ...
    info_region = memory_region_create( "VESA mode info", PAGE_SIZE, REGION_KERNEL | REGION_READ | REGION_WRITE );
    ...
    memory_region_remap_pages( info_region, 0x2000 );
    vesa_mode_info = ( vesa_mode_info_t* )info_region->address;
    ...
    memcpy( mode_info, vesa_mode_info, sizeof( vesa_mode_info_t ) );
    error = 0;
